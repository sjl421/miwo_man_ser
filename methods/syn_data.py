...
def syn_data_mysql(data):
	...
	# if data.has_key("table") and data.has_key("key") and data.has_key("item_list"):
	if "table" in data.keys() and "key" in data.keys() and "item_list" in data.keys():
		...
		for item in data["item_list"]:
			...
			if item[data["key"]] not in r_keys:
				...
				# ks = ks[: -2]
				# vl = vl[: -2]
				ks = ks + "insert_time_stamp"
				vl = vl + str(now_time_stamp)
		...
		for item in data["item_list"]:
			...
			if not operator.eq(item, r_list[item[data["key"]]]):
				...
				sql = sql + "update_time_stamp = " + str(now_time_stamp) + " where id = " + str(temp_id)
